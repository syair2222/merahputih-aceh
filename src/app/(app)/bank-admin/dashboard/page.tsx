...
import { ArrowLeft, ShieldAlert, Loader2, Building } from 'lucide-react';
...
export default function BankAdminDashboardPage() {
  ...
  useEffect(() => {
    if (!loading && user && user.role !== 'bank_partner_admin') {
    ...
    <div className="space-y-8">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Ini adalah dasbor khusus untuk admin mitra bank.</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground">Fitur untuk meninjau pengajuan fasilitas anggota dan data terkait akan segera tersedia di sini.</p>
          <div className="mt-6 flex justify-center">
            <Building className="h-32 w-32 text-muted opacity-20" />
          </div>
        </CardContent>
      </Card>
    </div>
